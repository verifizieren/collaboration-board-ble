...
class SSBmsgTypes(val tremolaState: TremolaState) {
    ...
    fun mkCustomApp(appId: String, payloadJson: String): String {
        val content = JSONArray()
        content.put("CUS")
        content.put(appId)
        content.put(JSONTokener(payloadJson).nextValue())
        return mkWire(content)
    }
    ...
    fun jsonToLogEntry(json: String, raw: ByteArray): LogEntry? {
        ...
        try {
            ...
            try {
                ...
                public = vTree.get("content").toString()
                confid = null
            }
            return LogEntry(
                key, author,
