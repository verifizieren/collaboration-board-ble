...
var bleState = {status: "BLE starting", peers: 0, queued: 0};
...
function load_peer_list() {
    var i, lst = '', row;
    lst += "<div style='padding: 0px 5px 10px 5px;'>";
    lst += "<button class='flat buttontext' style='border-radius: 25px; width: 50px; height: 50px; margin-right: 0.75em;' onclick='backend(\"ble:kick\");'>BLE</button>";
    lst += "<button class='chat_item_button light' style='overflow: hidden; width: calc(100% - 4em);' onclick='backend(\"ble:kick\");'>";
    lst += "<div style='white-space: nowrap;'><div style='text-overflow: ellipsis; overflow: hidden;'>Bluetooth LE sync</div>";
    lst += "<div style='text-overflow: clip; overflow: ellipsis;'><font size=-2>" + escapeHTML(bleState.status);
    lst += " · peers " + bleState.peers + " · queue " + bleState.queued + "</font></div></div></button></div>";
    for (i in localPeers) {
...
function backend(cmdStr) { // send this to Kotlin (or simulate in case of browser-only testing)
    ...
    if (cmdStr[0] === 'ready')
        ...
        b2f_new_event(e)
    } else if (cmdStr[0] === 'customApp:writeEntry') {
        var appId = cmdStr[1];
        var payload = JSON.parse(cmdStr.slice(2).join(' '));
        var e = {
            'header': {
                'tst': Date.now(),
                'ref': Math.floor(1000000 * Math.random()),
            ...
            'public': ['CUS', appId, payload],
            'confid': null
        };
        cbrowser_custom_app_log().push(e);
        b2f_new_event(e);
    } else if (cmdStr[0] === 'customApp:readEntries') {
        var replayAppId = cmdStr[1];
        var replayLimit = parseInt(cmdStr[2] || '0', 10);
        var replay = cbrowser_custom_app_log().filter(function (ev) {
            return ev.public && ev.public[0] === 'CUS' && ev.public[1] === replayAppId;
        });
        if (replayLimit > 0 && replay.length > replayLimit) {
            replay = replay.slice(replay.length - replayLimit);
        }
        replay.forEach(function (ev) { b2f_new_event(ev); });
    } else {
        // console.log('backend', JSON.stringify(cmdStr))
    }
}

function cbrowser_custom_app_log() {
    if (typeof tremola.customAppLog == "undefined") {
        tremola.customAppLog = [];
    }
    return tremola.customAppLog;
}
...
function b2f_ble_status(status, peers, queued) {
    bleState = {status: status, peers: peers, queued: queued};
    if (document.getElementById('the:connex'))
        load_peer_list();
}
...
function b2f_new_event(e) { // incoming SSB log event: we get map with three entries
    ...
    if (e.confid && e.confid.type === 'post') {
        ...
        var customAppData = null;
        if (Array.isArray(e.public) && e.public[0] === 'CUS') {
            customAppID = e.public[1];
            customAppData = e.public[2];
        } else if (e.public.type === 'CUS') {
            customAppID = e.public.app;
            customAppData = e.public.payload;
        }
        if (customAppID && window.miniApps && window.miniApps[customAppID]) {
            window.miniApps[customAppID].handleRequest("incoming_notification", {
                args: [customAppData],
                header: e.header
            });
        }
