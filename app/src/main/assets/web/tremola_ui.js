...
var currentMiniAppID = null;

var display_or_not = [
    'div:qr', 'div:back',
    'core', 'lst:chats', 'lst:posts', 'lst:contacts', 'lst:miniapps', 'lst:members', 'the:connex',
...
var main_scenarios = ['chats', 'miniapps', 'contacts', 'connex'];

var buttonList = ['btn:chats', 'btn:posts', 'btn:miniapps', 'btn:contacts', 'btn:connex'];
...
function onBackPressed() {
    ...
    if (curr_scenario === 'customApp') {
        if (currentMiniAppID && window.miniApps && window.miniApps[currentMiniAppID]) {
            window.miniApps[currentMiniAppID].handleRequest("onBackPressed", { args: [] });
        } else {
...
function setScenario(new_scenario) {
    ...
    if (new_scenario.indexOf('customApp:') === 0) {
        currentMiniAppID = new_scenario.substring('customApp:'.length);
        prev_scenario = 'miniapps';
        curr_scenario = 'customApp';
        return;
    }
    ...
    if (list_of_elements) {
        ...
        if (scenarioDisplay[curr_scenario] && scenarioDisplay[curr_scenario].indexOf('div:footer') >= 0) {
            let oldButton = document.getElementById('btn:' + curr_scenario);
            if (oldButton) {
                let cl = oldButton.classList;
        ...
        if (new_scenario !== 'customApp') {
            currentMiniAppID = null;
        }
        curr_scenario = new_scenario;
        if (scenarioDisplay[curr_scenario].indexOf('div:footer') >= 0) {
            var newButton = document.getElementById('btn:' + curr_scenario);
            if (newButton) {
                var cl = newButton.classList;
